...
open class LineChartFilledViewController: DemoBaseViewController
{
    ...
    override open func viewDidLoad()
    {
        ...
        let url = Bundle.main.url(forResource: "range", withExtension: "json")
        let data = NSData(contentsOf: url!)
        json = JSON(data : data! as Data,  options: .mutableContainers)
        ...
        xAxis.valueFormatter = DateValueFormatter(miniTime: json[0][0].doubleValue / 1000, interval: interval)
        xAxis.labelCount = 7
    ...
    func setDataCount()
    {
        ...
        let miniDate = json[0][0].doubleValue
        
        for i in 0..<json.count
        {
            let x  = ((json[i][0].doubleValue - miniDate) / interval) / 1000
            let y1 = json[i][1].doubleValue
            let y2 = json[i][2].doubleValue
