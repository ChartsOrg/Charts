 ...
 */
open class CustomCombinedChartRenderer : BarLineScatterCandleBubbleRenderer
{
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
                ...
                prepareBarHighlight(x: e.x, y1:100-(high.y-y1), y2:y1, barWidthHalf: barData.barWidth / 2, trans: trans, rect: &barRect)
