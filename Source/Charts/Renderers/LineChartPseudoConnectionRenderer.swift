//
//  LineChartPseudoConnectionRenderer.swift
//  Charts
//
...
class LineChartPseudoConnectionRenderer: LineChartRenderer
{
    ...
    open override func drawCubicBezier(context: CGContext, dataSet: ILineChartDataSet) {
        ...
        if _xBounds.range >= 1
        {
            ...
            for j in stride(from: firstIndex, through: lastIndex, by: 1)
            {
                ...
                if cur.x - prev.x > 1 {
                    drawCubicSave(context: context, dataSet: dataSet, cubicPath: cubicPath, matrix: valueToPixelMatrix, color: pseudoColor, boundsMin: j - 1, boundsRange: 1)
                }
                else{
        ...
        { // only one color per dataset
            ...
            if e1 != nil
            {
                ...
                for x in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
                {
                    ...
                    if e2.x - e1.x > 1 {
                        context.setStrokeColor(pseudoColor.cgColor)
                    }
                    else{
    ...
    private func  drawCubicSave(context: CGContext,
    ...
    private func drawLinearFill(context: CGContext, dataSet: ILineChartDataSet, trans: Transformer, min: Int,
                             range: Int)
    {
        guard let dataProvider = dataProvider else { return }
