...
{
    // 由DataProvider提供圓角對象、圓角數值
    func drawRect(context: CGContext, barRect: CGRect) {
        let bezierPath = UIBezierPath(roundedRect: barRect, byRoundingCorners: dataProvider?.corners ?? [], cornerRadii: .init(width: dataProvider?.cornerRadius ?? 0, height: dataProvider?.cornerRadius ?? 0))
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        if dataProvider.isDrawBarShadowEnabled
        {
            for j in stride(from: 0, to: buffer.rects.count, by: 1)
            {
                ...
                drawRect(context: context, barRect: barRect)
            }
        }
