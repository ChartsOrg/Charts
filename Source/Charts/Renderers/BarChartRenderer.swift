...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    {
        guard let barData = dataProvider?.barData else { return buffers.removeAll() }
        ...
        buffers = zip(buffers, barData.dataSets).map { buffer, set -> Buffer in
            ...
            if containsStacks, let vals = e.yValues
            {
                ...
                for value in vals
                {
                    ...
                    let barRect = CGRect(x: left, y: top,
                                         width: right - left,
                                         height: bottom - top)
                    buffers[index][bufferIndex] = barRect
                    bufferIndex += 1
                }
            }
            else
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: BarChartDataSetProtocol, index: Int)
    {
        ...
        trans.rectValuesToPixel(&buffers[index])
