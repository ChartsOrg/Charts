...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    @objc open func drawHorizontalBezier(context: CGContext, dataSet: LineChartDataSetProtocol)
    {
        ...
        var colorsAndPoints = [(color: NSUIColor, points: [CGPoint])]()
        ...
        if _xBounds.range >= 1
        {
            ...
            for j in _xBounds.dropFirst()
            {
                ...
                let nextColor = j < colors.count ? colors[j] : colors.last!

                points.append(point)

                if color != nextColor {
                    colorsAndPoints.append((
                        color: color,
                        points: points
                    ))
                    color = nextColor
        ...
        {
            for (color, points) in colorsAndPoints {
                context.saveGState()
                ...
                let clipRect = CGRect(
                    ...
                    width: maxX - minX,
                    height: maxY - minY
                ).applying(valueToPixelMatrix).insetBy(
                    dx: -dataSet.lineWidth / 2,
                    dy: -dataSet.lineWidth / 2
                )
