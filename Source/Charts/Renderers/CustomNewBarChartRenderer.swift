//
//  CustomNewBarChartRenderer.swift
//  Charts
//
//  Created by Oussema Ayed on 10/22/19.
//
...
import UIKit
#endif
public class CustomNewBarChartRenderer: BarChartRenderer {
    ...
    public override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
                ...
                var rect:CGRect = CGRect(x: e.x-0.3  , y: 0 , width: ((barData.barWidth / 2) ) + 0.5  , height: 100 )
                ...
                prepareBarHighlight(x: e.x , y1: 100  , y2: y2, barWidthHalf: (barData.barWidth / 2) * 2.5 , trans: trans, rect: &barRect)
