...
open class BlockChartDataSet: BarLineScatterCandleBubbleChartDataSet, IBarChartDataSet
{
    private func initialize()
    {
        self.highlightColor = NSUIColor.black
        
        self.calcStackSize(entries: entries as! [BlockChartDataEntry])
        self.calcEntryCountIncludingStacks(entries: entries as! [BlockChartDataEntry])
    }
    ...
    private func calcEntryCountIncludingStacks(entries: [BlockChartDataEntry])
    {
        _entryCountStacks = 0
    ...
    private func calcStackSize(entries: [BlockChartDataEntry])
    {
        for i in 0 ..< entries.count
        {
            if let vals = entries[i].yValues
    ...
    {
        guard let e = e as? BlockChartDataEntry
            else { return }
