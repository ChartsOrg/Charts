...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    @objc public var clipValuesToContentEnabled = false
    ...
    /// **default**: false
    @objc public var keepPositionOnRotation = false
    ...
    private var _isCustomViewPortEnabled = false
    ...
    internal func calculateLegendOffsets(offsetLeft: inout CGFloat, offsetTop: inout CGFloat, offsetRight: inout CGFloat, offsetBottom: inout CGFloat)
    {
        ...
        if _legend !== nil && _legend.isEnabled && !_legend.isDrawInsideEnabled
        {
    ...
    // FIXME: This variable is not used in the file.
    //    fileprivate var _panGestureReachedEdge = false
    ...
    public func gestureRecognizerShouldBegin(gestureRecognizer: NSGestureRecognizer) -> Bool
{
    ...
    {
        _isCustomViewPortEnabled = false
        calculateOffsets()
    }
    ...
    @objc open func setScaleEnabled(_ enabled: Bool)
    {
        if isScaleXEnabled != enabled || isScaleYEnabled != enabled
        {
            ...
            #if !os(tvOS)
                _pinchGestureRecognizer.isEnabled = _isPinchZoomEnabled || isScaleXEnabled || isScaleYEnabled
            #endif
        }
    }
    @objc public var isScaleXEnabled: Bool {
        ...
        @objc(setScaleXEnabled:) set {
            ...
            #if !os(tvOS)
                _pinchGestureRecognizer.isEnabled = _isPinchZoomEnabled || _isScaleXEnabled || _isScaleYEnabled
            #endif
        }
    }
    ...
    private var _isScaleYEnabled = true
    ...
    @objc public var isDoubleTapToZoomEnabled: Bool
    {
        get
        {
            return _isDoubleTapToZoomEnabled
        }
        @objc(setDoubleTapToZoomEnabled:) set
        {
            if _isDoubleTapToZoomEnabled != newValue
            {
                _isDoubleTapToZoomEnabled = newValue
                _doubleTapGestureRecognizer.isEnabled = _isDoubleTapToZoomEnabled
            }
        }
    }
    private var _isDoubleTapToZoomEnabled = true
    ...
    private var _isHighlightPerDragEnabled = true
    ...
    private var _isDrawGridBackgroundEnabled = false
    ...
    private var _isDrawBordersEnabled = false
    ...
    @objc public var isPinchZoomEnabled: Bool {
        ...
        {
            _isPinchZoomEnabled = newValue
            #if !os(tvOS)
                _pinchGestureRecognizer.isEnabled = _isPinchZoomEnabled || isScaleXEnabled || isScaleYEnabled
            #endif
        }
    }
    private var _isPinchZoomEnabled = false
    ...
    private var _isAutoScaleMinMaxEnabled = false
